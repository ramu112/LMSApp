(function(module) {
  mifosX.controllers = _.extend(module, {
    InventoryController: function(scope,webStorage, routeParams, location,$modal, resourceFactory, paginatorService,route) {
        ...
        scope.officeDatas = [];scope.itemMasterDatas = [];scope.manufacturerDatas = [];
				  		...
				  		scope.searchSupplier = function(filterText) {
							...
							scope.editQuality = function(itemId,valueQuality,provisionalserialNum,serialNumber,units,quantity){
					          ...
					          scope.searchStatusDetails = function(offset, limit, callback) {
					        	  if(scope.source == 'ALL')
						    	  resourceFactory.itemDetailsResource.getAlldetails({offset: offset, limit: limit,officeName : scope.officeName,manufacturer : scope.manufacturer} , callback);
					        	  else
					        		  resourceFactory.itemDetailsResource.getAlldetails({offset: offset, limit: limit , 
							    		  sqlSearch: scope.source ,officeName : scope.officeName,itemCode : scope.manufacturer} , callback);
						      };
